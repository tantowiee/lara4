	...
	<form class="form-horizontal" method="post" action="@if (isset($user)){{ URL::to('admin/users/' . $user->id . '/edit') }}@endif" autocomplete="off">
		...
		<div class="tab-content">
			...
			<div class="tab-pane active" id="tab-general">
				...
				<!-- Gender -->
				<div class="form-group {{{ $errors->has('gender2') || $errors->has('gender') ? 'error' : '' }}}">
					<label class="col-md-2 control-label" for="gender">Gender</label>
					<div class="col-md-6">
						@if ($mode == 'create')
							<select class="form-control" name="gender" id="gender">
								<option value="M"{{{ (Input::old('gender', 0) === 'M' ? ' selected="selected"' : '') }}}>{{{ Lang::get('general.male') }}}</option>
								<option value="F"{{{ (Input::old('gender', 0) === 'F' ? ' selected="selected"' : '') }}}>{{{ Lang::get('general.female') }}}</option>
							</select>
						@else
							<select class="form-control" >
								<option value="M"{{{ ($user->gender2 ? ' selected="selected"' : '') }}}>{{{ Lang::get('general.male') }}}</option>
								<option value="F"{{{ ( ! $user->gender2 ? ' selected="selected"' : '') }}}>{{{ Lang::get('general.female') }}}</option>
							</select>
						@endif
